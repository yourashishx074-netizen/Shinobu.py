...
API_HASH = os.getenv("API_HASH", "e62c39963de4721efb786f7cfc64022")
BOT_TOKEN = os.getenv("BOT_TOKEN", "8836214965:AAG4en9WMrHio9z9kJOC-eq_QvR")
OWNER_ID = int(os.getenv("OWNER_ID", "888229763"))
MONGO_URL = os.getenv("MONGO_URL", "mongodb+srv://Elevenyts:Elevenyts@cluster...")
LOGGER_ID = int(os.getenv("LOGGER_ID", "-1004380807747"))
PORT = int(os.getenv("PORT", "8080"))

YOUTUBE_API_KEYS = [
    os.getenv("YOUTUBE_API_KEY_1", "AIzaSyA-1__eBr2z2h3V1CNCraU3OSMmWyIvgbS..."),
...
async def set_menu_commands():
    commands = [
        BotCommand("start", "ꜱᴛᴀʀᴛꜱ ᴛʜᴇ ᴍᴜsɪᴄ ʙᴏᴛ."),
...
/end or /stop : ᴄʟᴇᴀʀꜱ ᴛʜᴇ Qᴜᴇᴜᴇ ᴀɴᴅ ᴇɴᴅ ᴛʜᴇ ꜱᴛʀᴇᴀᴍ.
...
/channelplay [CHAT USERNAME OR ID] OR [DISABLE] : ᴄᴏɴɴᴇᴄᴛ ᴄʜᴀɴɴᴇʟ ᴛᴏ ᴀ ɢʀᴏᴜᴘ ᴀɴᴅ ꜱᴛᴀʀᴛs ꜱᴛʀᴇᴀᴍɪɴɢ ᴛʀᴀᴄᴋꜱ ʙʏ ᴛʜᴇ ʜᴇʟᴘ ᴏꜰ ᴄᴏᴍᴍᴀɴᴅꜱ ꜱᴇɴᴛ ɪɴ ɢʀᴏᴜᴘ.""",

    "gban": """**ɢʟᴏʙᴀʟ ʙᴀɴ ꜰᴇᴀᴛᴜʀᴇ [ᴏɴʟʏ ꜰᴏʀ ꜱᴜᴅᴏᴇʀꜱ] :**

/gban [USERNAME OR REPLY TO A USER] : ɢʟᴏʙᴀʟʟʏ ʙᴀɴꜱ ᴛʜᴇ ᴜꜱᴇʀ ꜰʀᴏᴍ ᴀʟʟ ᴛʜᴇ ꜱᴇʀᴠᴇᴅ ᴄʜᴀᴛꜱ ᴀɴᴅ ʙʟᴀᴄᴋʟɪꜱᴛ ʜɪᴍ ꜰʀᴏᴍ ᴜꜱɪɴɢ ᴛʜᴇ ʙᴏᴛ.
...
async def dynamic_login_steps(_, message: Message):
    ...
    if not message.text or user_id not in login_state or message.text.startswith("/"):
        return
...
async def play_handler(_, message: Message):
    ...
    try:
        ...
        if call_py:
            # PyTgCalls 3.x accepts a media URL directly.
            # If a call already exists, play() changes the stream.
...
async def stop_music(_, message: Message):
    if call_py:
        try:
            ...
            await message.reply_text(
                "⏹️ **Voice Chat se disconnect ho gaya aur stream end ho gayi.**"
            )
        except Exception as e:
...
async def main():
    ...
    # Send Logger Alert to your channel
    if LOGGER_ID:
        try:
            uptime, storage, cpu, ram = get_system_stats()
